    ...
    public sealed class MinimapSenderBroadcastService : IDisposable
    {
        ...
        public MinimapSenderBroadcastService(GamePlayerOwner gamePlayerOwner)
        {
            ...
            _timer.Elapsed += (sender, args) =>
            {
                ...
                {
                    MinimapSenderPlugin.MinimapSenderLogger.LogError($"Exception {e.GetType()} occurred. Message: {e.Message}. StackTrace: {e.StackTrace}");
                }
            };
        }
...
private void SendData()
{
    try
    {
        if (_gamePlayerOwner != null && _gamePlayerOwner.Player != null)
        {
            ...
            {
                airdrops = MinimapSenderPlugin.airdrops?.ToArray() ?? new Vector3[0];
            }
